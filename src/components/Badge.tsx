...
const Badge = ({ children, variant = "neutral" }: BadgeProps) => {
  const styles = {
    ...
    neutral: "bg-neutral-800/70 text-soft-white border border-white/10",
    outline: "bg-transparent text-soft-white border border-white/20",
  };

  return (
    <span
      className={`inline-flex items-center rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-wide ${styles[variant]}`}
    >
      {children}
    </span>
  );
